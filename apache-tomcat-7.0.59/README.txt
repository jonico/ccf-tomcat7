This file contains the instructions on how to run TeamForge Connector Server 2.3.5 aka CollabNet Connect/Sync in this tomcat
distribution.
...
TeamForge Connector Server 2.3.5 requires the following software to be installed on the computer
it is running on
...
TeamForge Connector Server 2.3.5 distribution and extracted it to the directory
...
2. Open the "ccf.conf" configuration file(e.g. $CATALINA_HOME/ccf.conf ), 
   ...
   and to be used by TeamForge Connector Server 2.3.5. 
...
2. change into the directory where you unzipped the TeamForge Connector Server 2.3.5 distribution:

   cd c:\path\to\apache-tomcat-7.0.59
    
3. To install the service, run the following command:
   
   bin\service.bat
   
   This will register TeamForge Connector Server 2.3.5 as a windows service and set it up
...
To run the TeamForge Connector Server 2.3.5 distribution, you can start the service using the
...
2. In addition to the settings specified in ccf.conf and ccfhomeruntimeconfig.properties 
   ...
   unzipped the TeamForge Connector Server 2.3.5 distribution
   and the location of your JRE:

        CATALINA_HOME="/opt/ccfmaster/apache-tomcat-7.0.59"
